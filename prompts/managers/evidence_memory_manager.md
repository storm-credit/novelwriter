...
4. **패턴 추적**: 반복 오류 패턴 탐지 (예: 특정 장에서 소환의 무게 묘사 반복 누락)
5. **메모리 인덱싱**: 다음 장 감사 시 컨텍스트로 제공할 known issues 정리
...
| 위치 힌트 | 장·씬·설정집 섹션 명시 | 없어도 허용 (메모로 요청) |

---

## D. 중복 병합 규칙

- **동일 장 + 동일 씬 + 동일 서사 원칙 위반**: 하나로 병합, confidence는 높은 쪽 채택
- **다른 장의 동일 패턴**: 별도 이슈 + 패턴 플래그 설정
...
```json
{
  ...
  "pattern_flags": [
    { "pattern": "소환의 무게(수명 소모) 묘사 누락 반복", "chapters": [3, 5], "frequency": 2 }
  ],
  ...
  "next_chapter_context": {
    ...
    "watch_flags": ["다음 장에서 특별히 주의할 요소들"]
  }
}
```
